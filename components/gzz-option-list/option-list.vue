<template>
  <view class="list-box">
    <view v-for="(item,index) in dataList" :key="index" class="item" :style="{height: (itemHeight - 1)+'rpx'}" @tap="tap(item)" @touchstart="touchstart(item,$event)" @touchmove="touchmove" @touchend="touchend()">
      <view class="option-box">
        <view v-for="oItem in options" :key="oItem.text" class="option-item" @tap.stop="optionClick(item,oItem)" :style="{
								  width: oItem.width?oItem.width+'rpx': '100rpx',
								  backgroundColor: oItem.bgColor }">
          <image :style="{width:'30px',height:'30px',display:'block'}" :src="oItem.src" mode=""></image>


          <text :style="{ color:'#ffffff'}">{{ oItem.text }}</text>
        </view>
      </view>
